...
如果一个对象不可能是环对象，即使它包含存在环对象的结构也不影响释放操作，因为非环对象的行为是和 std::shared_ptr 保持一致的，在包含环对象的非环对象析构时会触发环对象对应的析构操作。  
  
一个非环指向包含环对象会让环对象的引用计数加一，而对于该类型的计数属于外部引用的范畴。也就是说，只要该非环对象存活，那么它指向的环对象一定存活。
  
对于属于同一**类型**环的对象，如果环对象 A 可以通向环对象 B，而 B 却不存在通往 A 的活路，那么 B 所属的环依赖 A 的存活，A 则依赖上层引用者的存活，而这种依赖从宏观上来看，环和环之间不存在环形依赖（否则它们就是同一个环），所以析构的步骤是一个环消亡后导致另一个环的动摇。
...
## 复杂度说明
该算法无内存移动，无分代，无内存分区，无世界暂停。  
对于内存碎片问题，已经在上文提供良好内存管理算法的链接了，正在进行的基于 Mix-C/MMU 的硬件级内存管理单元可以彻底解决碎片问题。  
对于非环对象，该算法的复杂度与 std::shared_ptr 一致  
对于环对象，复杂度等于环上节点的个数  

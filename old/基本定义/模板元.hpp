...
命名空间 模板元空间{
    ...
    结构体 类型{
        模板<类 乙>
        结构体 和类型{
            静态 符号常量 布尔 相同 = 模板元空间::类型相同<甲, 乙>::结果;
            静态 符号常量 布尔 不同 = 模板元空间::类型相同<甲, 乙>::结果 == 否;
        };

        静态 符号常量 布尔 是枚举     = 模板元空间::是枚举<甲>::结果;
        静态 符号常量 布尔 是指针     = 模板元空间::是指针<甲>::结果;
        静态 符号常量 布尔 是引用     = 模板元空间::是引用<甲>::结果;
        静态 符号常量 布尔 是非寄     = 模板元空间::是非寄<甲>::结果;
        静态 符号常量 布尔 是常量     = 模板元空间::是常量<甲>::结果;
        静态 符号常量 布尔 是整数     = 模板元空间::是整数<甲>::结果;
        静态 符号常量 布尔 是整型     = 模板元空间::是整型<甲>::结果;
        静态 符号常量 布尔 是数值     = 模板元空间::是数值<甲>::结果;
        静态 符号常量 布尔 是联合体   = 模板元空间::是联合体<甲>::结果;
        静态 符号常量 布尔 是结构体   = 模板元空间::是结构体<甲>::结果;
        静态 符号常量 布尔 是自然数   = 模板元空间::是自然数<甲>::结果;
        静态 符号常量 布尔 是浮点数   = 模板元空间::是浮点数<甲>::结果;
        静态 符号常量 布尔 不可继承   = 模板元空间::是不可继承<甲>::结果;
...
模板<类 甲>
结构体 类型 : 模板元空间::类型<甲>{};
模板<> 结构体 类型<I08> : 模板元空间::类型<I08>{
    静态 符号常量 I08        最大值 = I08(0x7f);
    静态 符号常量 I08        最小值 = I08(0x80);
    静态 符号常量 阿斯克码 * 最大值字符串化 = "127";
    静态 符号常量 阿斯克码 * 最小值字符串化 = "-128";
};
模板<> 结构体 类型<I16> : 模板元空间::类型<I16>{
    静态 符号常量 I16        最大值 = I16(0x7fff);
    静态 符号常量 I16        最小值 = I16(0x8000);
    静态 符号常量 阿斯克码 * 最大值字符串化 = "32767";
    静态 符号常量 阿斯克码 * 最小值字符串化 = "-32768";
};
模板<> 结构体 类型<I32> : 模板元空间::类型<I32>{
    静态 符号常量 I32        最大值 = I32(0x7fffffff);
    静态 符号常量 I32        最小值 = I32(0x80000000);
    静态 符号常量 阿斯克码 * 最大值字符串化 = "2147483647";
    静态 符号常量 阿斯克码 * 最小值字符串化 = "-2147483648";
};
模板<> 结构体 类型<I64> : 模板元空间::类型<I64>{
    静态 符号常量 I64        最大值 = I64(0x7fffffffffffffff);
    静态 符号常量 I64        最小值 = I64(0x8000000000000000);
    静态 符号常量 阿斯克码 * 最大值字符串化 = "9223372036854775807";
    静态 符号常量 阿斯克码 * 最小值字符串化 = "-9223372036854775808";
};
模板<> 结构体 类型<U08> : 模板元空间::类型<U08>{
    静态 符号常量 U08        最大值 = U08(0xff);
    静态 符号常量 U08        最小值 = U08(0);
    静态 符号常量 阿斯克码 * 最大值字符串化 = "255";
    静态 符号常量 阿斯克码 * 最小值字符串化 = "0";
};
模板<> 结构体 类型<U16> : 模板元空间::类型<U16>{
    静态 符号常量 U16        最大值 = U16(0xffff);
    静态 符号常量 U16        最小值 = U16(0);
    静态 符号常量 阿斯克码 * 最大值字符串化 = "65535";
    静态 符号常量 阿斯克码 * 最小值字符串化 = "0";
};
模板<> 结构体 类型<U32> : 模板元空间::类型<U32>{
    静态 符号常量 U32        最大值 = U32(0xffffffff);
    静态 符号常量 U32        最小值 = U32(0);
    静态 符号常量 阿斯克码 * 最大值字符串化 = "4294967295";
    静态 符号常量 阿斯克码 * 最小值字符串化 = "0";
};
模板<> 结构体 类型<U64> : 模板元空间::类型<U64>{
    静态 符号常量 U64        最大值 = U64(0xffffffffffffffff);
    静态 符号常量 U64        最小值 = U64(0);
    静态 符号常量 阿斯克码 * 最大值字符串化 = "18446744073709551615";
    静态 符号常量 阿斯克码 * 最小值字符串化 = "0";
};
模板<> 结构体 类型<F32>{
    静态 符号常量 F32        最大值 = (F32)3.40282347e+38;
    静态 符号常量 F32        最小值 = (F32)-3.40282347e+38;
    静态 符号常量 阿斯克码 * 最大值字符串化 = "3.40282347e+38";
    静态 符号常量 阿斯克码 * 最小值字符串化 = "-3.40282347e+38";
};
模板<> 结构体 类型<F64>{
    静态 符号常量 F64        最大值 = 1.7976931348623157e+308;
    静态 符号常量 F64        最小值 = -1.7976931348623157e+308;
    静态 符号常量 阿斯克码 * 最大值字符串化 = "1.7976931348623157e+308";
    静态 符号常量 阿斯克码 * 最小值字符串化 = "-1.7976931348623157e+308";
};
...
模板<布尔 条件> 结构体 假如{
    ...
    结构体 选{
        模板<类 乙>
        结构体 否则选{
            定义 类型名称 模板元空间::选择<条件, 甲, 乙>::结果 作为结果;
        };
    };
    模板<类 类型>
    结构体 取{
        ...
        结构体 值{
            模板<类型 乙>
            结构体 否则取{
                静态 符号常量 类型 作为结果 = 模板元空间::选择值<条件, 类型, 甲, 乙>::结果;
            };
        };
    };
};
模板<类 甲>
结构体 函数{
    定义 类型名称 模板元空间::函数返回值<甲>::类型 返回值类型;
    模板<整数 下标>
    结构体 参数{
        定义 类型名称 模板元空间::函数参数<甲, 下标>::类型 类型;
    };
    静态 符号常量 自然数 参数个数 = 模板元空间::函数参数个数<甲>::结果;
};
模板<类 甲, 类 ... 剩余参数>
结构体 从{
    模板<自然数 下标>
    结构体 选择类型{
        定义 类型名称 模板元空间::参数<下标, 甲, 剩余参数...>::类型 作为结果;
    };
    模板<甲 首参, 甲 ... 列表>
    结构体 值{
        模板<自然数 下标>
        结构体 选择{
            静态 符号常量 甲 作为结果 = 模板元空间::列表<下标, 甲, 首参, 列表...>::作为结果;
        };
    };
};

模板<类 甲>
引用 纯元素 = 类型名称 类型<甲>::模板 移除<引用标记 | 常量标记>::作为结果;

模板<类 ... 元素列表>
符号常量 自然数 参数个数(元素列表 常量 & ...列表) {
    回递 参数数(元素列表);
}

模板<类 甲, 类 ... 元素列表>
符号常量 甲 首参(甲 常量 & 首参, 元素列表 常量 & ...列表) {
    回递 首参;
}

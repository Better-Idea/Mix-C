...
#define 正遍历(名称,对象,...)                                                               \
    for(推导类型 常量 & _0 = 对象, * _1 = 取址(_0); _1 != 空值; _1 = 空值)                  \
    for(推导类型 名称 = _0->正遍历器(__VA_ARGS__); 名称.未达右边界(); 名称 = 名称.下一个())
#define 反遍历(名称,对象,...)                                                               \
    for(推导类型 常量 & _0 = 对象, * _1 = 取址(__临时__); _1 != 空值; _1 = 空值)            \
    for(推导类型 名称 = _0->反遍历器(__VA_ARGS__); 名称.未达左边界(); 名称 = 名称.下一个())
...
模板<类 浮点类型, 类 等效类型, 自然数 小数位数, 自然数 阶码位数, 自然数 阶码偏移>
联合体 MFXX{
    定义 MFXX<浮点类型, 等效类型, 小数位数, 阶码位数, 阶码偏移> * MFXXP;
    结构体{
        等效类型 小数 : 小数位数;
        等效类型 阶码 : 阶码位数;
        等效类型 符号 : 1;
    ...
    符号常量 MFXX(等效类型 符号, 等效类型 阶码, 等效类型 小数) : 
        符号(符号), 阶码(阶码), 小数(小数) {
    }
    运算重载 浮点类型() 常量 {
        回递 值;
...
定义 MFXX<F32, U32, 23, 8,  127 > MF32;
定义 MFXX<F64, U64, 52, 11, 1023> MF64;
...
符号常量 推导类型              等于     = 比较结果结构<迷你空间::等于    >();
符号常量 推导类型              不等于   = 比较结果结构<迷你空间::不等于  >();
符号常量 推导类型              小于     = 比较结果结构<迷你空间::小于    >();
符号常量 推导类型              小于等于 = 比较结果结构<迷你空间::小于等于>();
符号常量 推导类型              大于     = 比较结果结构<迷你空间::大于    >();
符号常量 推导类型              大于等于 = 比较结果结构<迷你空间::大于等于>();
符号常量 空引用结构            空引用{};
符号常量 非数结构              非数{};
...
#define 生成(类型)                                                  \
模板<> 内联链接 整数 比较(类型 常量 & 左, 类型 常量 & 右) {         \
    回递 左 == 右 ? 整数(等于) : 左 > 右 ? 整数(大于) : 整数(小于); \
}

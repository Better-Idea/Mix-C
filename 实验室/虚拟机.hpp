...
结构体 处理器{
    ...
    处理器(空指针 MEM, U64 BYTES){
        ...
        $M(SHL){
            八相运算($X(带进位){
                $F1(U, C.U > 空间量(整数器), <<, 空间量(C) - C.U, | STA.CF);
            });
        };
        $M(SHR){
            八相运算($X(){
                $F1(U, C.U > 空间量(整数器), >>, C.U - 1);
            });
        };
        $M(SAR){
            八相运算($X(){
                // S                            CF
                // 1110 1111 >> 4 -> 1111 1110 [1]
                // 1110 1111 >> 5 -> 1111 1111 [0]
                // 1110 1111 >> 6 -> 0000 0000 [0]
                $F1(I, C.U > 指令集::最高复位位索引(B.U) + 1, >>, C.U - 1);
            });
        };
        $M(ADD){
            四相运算($X(带进位){

...
命名空间 测试{
    ...
    结构体 原始串{
        ...
        标头(截取)
            ...
            断言(A == A->截取(0, -1, $F[](自然数 长度){
                缓冲区[长度] = '\0';
                回递 缓冲区;
            }));

            断言(_"123456789abcdef" == A->截取(1, -1, $F[](自然数 长度){
                缓冲区[长度] = '\0';
                回递 缓冲区;
            }));

            断言(_"f" == A->截取(-1, -1, $F[](自然数 长度) {
                缓冲区[长度] = '\0';
                回递 缓冲区;
            }));
            
            断言(_"123" == A->截取(1, 3, $F[](自然数 长度) {
                缓冲区[长度] = '\0';
                回递 缓冲区;
            }));
            
            断言(_"321" == A->截取(3, 1, $F[](自然数 长度) {
                缓冲区[长度] = '\0';
                回递 缓冲区;
            }));

            断言(_"3456789abcde" == A->截取(3, -2, $F[](自然数 长度) {
                缓冲区[长度] = '\0';
                回递 缓冲区;
            }));
        $
        ...
        标头(函数运算重载)
            被测结构 A = _"0123456789";
            断言(_"1" == A(1, 1, $F[](自然数 长度) {
                缓冲区[长度] = '\0';
                回递 缓冲区;
            }));

            断言(_"12" == A(1, 2, $F[](自然数 长度) {
                缓冲区[长度] = '\0';
                回递 缓冲区;
            }));

            断言(_"123456789" == A(1, A->长度 - 1, $F[](自然数 长度) {
                缓冲区[长度] = '\0';
                回递 缓冲区;
            }));

            断言(_"123456789" == A(1, -1, $F[](自然数 长度) {
                缓冲区[长度] = '\0';
                回递 缓冲区;
            }));

            断言(_"76543210" == A(-3, 0, $F[](自然数 长度) {
                缓冲区[长度] = '\0';
                回递 缓冲区;
            }));

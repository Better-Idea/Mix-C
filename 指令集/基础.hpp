...
命名空间 指令集 {
        ...
        内联链接 自然数 低位一个数(U64 值) {
            回递 低位零个数(~值);
        }
        内联链接 自然数 高位一个数(U64 值) {
            回递 高位零个数(~值);
        }
    ...
    内联链接 布尔 位测试(U32 值, 自然数 下标) {
        回递 _bittest((long *)& 值, long(下标)) != 0;
    }
    内联链接 布尔 位测试(U16 值, 自然数 下标) {
        U32 临时 = 值;
        回递 _bittest((long *)& 临时, long(下标)) != 0;
    }
    内联链接 布尔 位测试(U08 值, 自然数 下标) {
        U32 临时 = 值;
        回递 _bittest((long*)& 临时, long(下标)) != 0;
    }
    内联链接 布尔 位测试后位置位(U32 & 值, 自然数 下标) {
        回递 _bittestandset((long *)& 值, long(下标)) != 0;
    }
    内联链接 布尔 位测试后位置位(U16 & 值, 自然数 下标) {
        U32  临时 = 值;
        布尔 结果 = _bittestandset((long*)& 临时, long(下标)) != 0;
        值 = (U16)临时;
        回递 结果;
    }
    内联链接 布尔 位测试后位置位(U08 & 值, 自然数 下标) {
        U32  临时 = 值;
        布尔 结果 = _bittestandset((long*)& 临时, long(下标)) != 0;
        值 = (U08)临时;
        回递 结果;
    }
    内联链接 布尔 位测试后位复位(U32 & 值, 自然数 下标) {
        回递 _bittestandreset((long *)& 值, long(下标)) != 0;
    }
    内联链接 布尔 位测试后位复位(U16 & 值, 自然数 下标) {
        U32  临时 = 值;
        布尔 结果 = _bittestandreset((long*)& 临时, long(下标)) != 0;
        值 = (U16)临时;
        回递 结果;
    }
    内联链接 布尔 位测试后位复位(U08 & 值, 自然数 下标) {
        U32  临时 = 值;
        布尔 结果 = _bittestandreset((long*)& 临时, long(下标)) != 0;
        值 = (U08)临时;
        回递 结果;
    }
    内联链接 布尔 位测试后位取反(U32 & 值, 自然数 下标) {
        回递 _bittestandcomplement((long *)& 值, long(下标)) != 0;
    }
    内联链接 布尔 位测试后位取反(U16 & 值, 自然数 下标) {
        U32  临时 = 值;
        布尔 结果 = _bittestandcomplement((long*)& 临时, long(下标)) != 0;
        值 = (U16)临时;
        回递 结果;
    }
    内联链接 布尔 位测试后位取反(U08 & 值, 自然数 下标) {
        U32  临时 = 值;
        布尔 结果 = _bittestandcomplement((long*)& 临时, long(下标)) != 0;
        值 = (U08)临时;
        回递 结果;
    ...
    内联链接 自然数 置位位个数(U32 值) {
        回递 (自然数)__popcnt(值);
    }
    内联链接 自然数 置位位个数(U16 值) {
        回递 (自然数)__popcnt((U32)值);
    }
    内联链接 自然数 置位位个数(U08 值) {
        回递 (自然数)__popcnt((U32)值);
    }
    内联链接 自然数 复位位个数(U32 值) {
        回递 (自然数)__popcnt(~值);
    }
    内联链接 自然数 复位位个数(U16 值) {
        回递 (自然数)__popcnt(~(U32)值);
    }
    内联链接 自然数 复位位个数(U08 值) {
        回递 (自然数)__popcnt(~(U32)值);
    }
    内联链接 自然数 低位零个数(U32 值) {
        回递 _tzcnt_u32(值);
    }
    内联链接 自然数 低位零个数(U16 值) {
        回递 _tzcnt_u32(值);
    }
    内联链接 自然数 低位零个数(U08 值) {
        回递 _tzcnt_u32(值);
    }
    内联链接 自然数 高位零个数(U32 值) {
        回递 _lzcnt_u32(值);
    }
    内联链接 自然数 高位零个数(U16 值) {
        回递 _lzcnt_u32(值);
    }
    内联链接 自然数 高位零个数(U08 值) {
        回递 _lzcnt_u32(值);
    }
    
    内联链接 自然数 最低置位位索引(U32 值) {
        unsigned long R;
    ...
    内联链接 自然数 最低置位位索引(U16 值) {
        回递 最低置位位索引((U32)值);
    }
    内联链接 自然数 最低置位位索引(U08 值) {
        回递 最低置位位索引((U32)值);
    }
    内联链接 自然数 最高置位位索引(U32 值) {
        unsigned long R;
    ...
    内联链接 自然数 最高置位位索引(U16 值) {
        回递 最高置位位索引((U32)值);
    }
    内联链接 自然数 最高置位位索引(U08 值) {
        回递 最高置位位索引((U32)值);
    }

    内联链接 自然数 最低复位位索引(U64 值) { 
        回递 最低置位位索引((U64)~值); 
    }
    内联链接 自然数 最低复位位索引(U32 值) { 
        回递 最低置位位索引((U32)~值); 
    }
    内联链接 自然数 最低复位位索引(U16 值) { 
        回递 最低置位位索引((U16)~值); 
    }
    内联链接 自然数 最低复位位索引(U08 值) { 
        回递 最低置位位索引((U08)~值); 
    }
    内联链接 自然数 最高复位位索引(U64 值) { 
        回递 最高置位位索引((U64)~值); 
    }
    内联链接 自然数 最高复位位索引(U32 值) { 
        回递 最高置位位索引((U32)~值); 
    }
    内联链接 自然数 最高复位位索引(U16 值) { 
        回递 最高置位位索引((U16)~值); 
    }
    内联链接 自然数 最高复位位索引(U08 值) { 
        回递 最高置位位索引((U08)~值); 
    }
    内联链接 U08 循环左移(U08 被移数, 自然数 移数) {
        回递 _rotl8(被移数, int(移数));
    }
    内联链接 U16 循环左移(U16 被移数, 自然数 移数) {
        回递 _rotl16(被移数, int(移数));
    }
    内联链接 U32 循环左移(U32 被移数, 自然数 移数) {
        回递 _rotl(被移数, int(移数));
    }
    内联链接 U64 循环左移(U64 被移数, 自然数 移数) {
        回递 _rotl64(被移数, int(移数));
    }
    内联链接 U08 循环右移(U08 被移数, 自然数 移数) {
        回递 _rotr8(被移数, int(移数));
    }
    内联链接 U16 循环右移(U16 被移数, 自然数 移数) {
        回递 _rotr16(被移数, int(移数));
    }
    内联链接 U32 循环右移(U32 被移数, 自然数 移数) {
        回递 _rotr(被移数, int(移数));
    }
    内联链接 U64 循环右移(U64 被移数, 自然数 移数) {
